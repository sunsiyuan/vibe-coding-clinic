...
本目录用于生成并展示「Vibe Coding 急诊室」嘉宾的 vibe coding 项目集合页。每个项目卡片按以下顺序展示：① **logo**（56px）② **项目名** ③ **简述**（最多 3 行）④ **访问项目**（若有）⑤ **Vibe Coding 急诊室复盘**（默认折叠，点击展开；两篇小红书：标题链接 + 摘要 1 行）。不展示类型与作者。

- **页面顶部**：使用 **clinic-logo.png** 作为项目墙的大 logo（居中展示），与标题「Vibe Coding 急诊室 · 嘉宾项目墙」一起呈现。替换 logo 时只需覆盖本目录下的 `clinic-logo.png` 并重新生成或直接刷新页面即可。
...
- 「Vibe Coding 急诊室复盘」默认折叠，点击展开；每篇：**标题（超链）** + **摘要（1 行）**；不展示金句与 workshop 文件列表
...
**小红书主账号**：在 `wall/config.json` 中配置 `xiaohongshu.account_name` 与 `xiaohongshu.account_url`，页面头部会展示对应链接。
